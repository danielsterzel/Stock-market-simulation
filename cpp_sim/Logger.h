...
// TODO: probably get rid of the isFileCreated File and use the checkIfFileOpenedProperly function.
...
public:
    ...
    void openFile(const std::string &filePath, const bool overwriteFile = false) {
        ...
        // }
        Error::checkIfFileOpenedProperly(file, filePath);
    ...
    static void saveScenarioConfig(const Agents &agentContainer, const std::string& filePath) {
        // TODO: fix this code duplication -> main has the same agent counting logic [ already fixed ]

        std::ofstream configFile;
        configFile.open(filePath);
        // if (not configFile.is_open()) {
        //     throw std::runtime_error("Failed to create scenario file: " + filePath);
        // }
        Error::checkIfFileOpenedProperly(configFile, filePath);
